...
use nn_phaseshifts, only: all_phaseshifts, lab_energy
...
!!
!> @brief      Saves list of phases as a function of t_lab
!!
!! Writes phase-shifts with error bars as a function of lab energy from 1 to 350 MeV.
!!
!!
!! @author     Raul L Bernal-Gonzalez
!! @author     Rodrigo Navarro Perez
!!
subroutine plot_phases_t_lab(potential, parameters, covariance, output_name)
    implicit none
    type(nn_model), intent(in) :: potential !< nn scattering model
    real(dp), intent(in), dimension(:) :: parameters !< potential parameters
    real(dp), intent(in), dimension(:, :) :: covariance !< Covariance matrix
    ...
    open(newunit=unit, file=trim(output_name)//'_pp_phases_t_lab.dat', status='unknown')
    ...
    open(newunit=unit, file=trim(output_name)//'_np_phases_t_lab.dat', status='unknown')
    ...
    close(unit)
end subroutine plot_phases_t_lab


!!
!> @brief      Saves list of phases as a function of k_cm
!!
!! Writes phase-shifts with error bars as a function of center of mass momentum
!! from 1 to 400 MeV.
!!
!!
!! @author     Raul L Bernal-Gonzalez
!! @author     Rodrigo Navarro Perez
!!
subroutine plot_phases_k_cm(potential, parameters, covariance, output_name)
    implicit none
    type(nn_model), intent(in) :: potential !< nn scattering model
    real(dp), intent(in), dimension(:) :: parameters !< potential parameters
    real(dp), intent(in), dimension(:, :) :: covariance !< Covariance matrix
    ...
    integer, parameter :: jmax = 4
    real(dp), parameter :: k_cm_min = 1._dp
    real(dp), parameter :: k_cm_max = 400._dp
    real(dp), parameter :: k_cm_step = 1._dp
    real(dp), dimension(1:5, 1:jmax) :: phases, phases_errors
    real(dp), allocatable, dimension(:, :, :) :: d_phases
    ...
    open(newunit=unit, file=trim(output_name)//'_pp_phases_k_cm.dat', status='unknown')
    write(unit, '(17a13)') 'k_cm', '1S0', 'sig_1S0', '1D2', 'sig_1D2', '3P0', 'sig_3P0', '3P1', 'sig_3P1', '3P2', &
        'sig_3P2', 'Ep2', 'sig_Ep2', '3F2', 'sig_3F2', '3F3', 'sig_3F3'
    k_cm = k_cm_min
    do
        if(k_cm > k_cm_max) exit
        t_lab = lab_energy(k_cm, 'pp')
        call all_phaseshifts(potential, parameters, t_lab, 'pp', phases, d_phases)
        phases = phases*180/pi
        d_phases = d_phases*180/pi
        do j=1, size(phases, 2)
            ...
            enddo
        enddo
        write(unit, '(17f13.6)') k_cm, phases(1, 1), phases_errors(1, 1), phases(1, 3), phases_errors(1, 3), &
            phases(5, 1), phases_errors(5, 1), phases(2, 2), phases_errors(2, 2), phases(3, 3), phases_errors(3, 3), &
            phases(4, 3), phases_errors(4, 3), phases(5, 3), phases_errors(5, 3), phases(2, 4), phases_errors(2, 4)
        k_cm = k_cm + k_cm_step
    end do
    ...
    open(newunit=unit, file=trim(output_name)//'_np_phases_k_cm.dat', status='unknown')
    write(unit, '(35a13)') 'k_cm', '1S0', 'sig_1S0', '3P0', 'sig_3P0', '1P1', 'sig_1P1', '3P1', 'sig_3P1', '3S1', &
        'sig_3S1', 'Ep1', 'sig_Ep1', '3D1', 'sig_3D1', '1D2', 'sig_1D2', '3D2', 'sig_3D2', '3P2', 'sig_3P2', 'Ep2', &
        'sig_Ep2', '3F2', 'sig_3F2', '1F3', 'sig_1F3', '3F3', 'sig_3F3', '3D3', 'sig_3D3', 'Ep3', 'sig_Ep3', '3G3', &
        'sig_3G3'
    k_cm = k_cm_min
    do
        ...
        do j=1, size(phases, 2)
            ...
            enddo
        enddo
        write(unit, '(35f13.6)') k_cm, phases(1, 1), phases_errors(1, 1), phases(5, 1), phases_errors(5, 1), &
            phases(1, 2), phases_errors(1, 2), phases(2, 2), phases_errors(2, 2), phases(3, 2), phases_errors(3, 2), &
            phases(4, 2), phases_errors(4, 2), phases(5, 2), phases_errors(5, 2), phases(1, 3), phases_errors(1, 3), &
            phases(2, 3), phases_errors(2, 3), phases(3, 3), phases_errors(3, 3), phases(4, 3), phases_errors(4, 3), &
            phases(5, 3), phases_errors(5, 3), phases(1, 4), phases_errors(1, 4), phases(2, 4), phases_errors(2, 4), &
    ...
    close(unit)
end subroutine plot_phases_k_cm
...
subroutine write_optimization_results(model, initial_parameters, parameters, mask, chi2, n_points, &
    ...
    call plot_phases_t_lab(model, parameters, covariance, output_name)
    call plot_phases_k_cm(model, parameters, covariance, output_name)
